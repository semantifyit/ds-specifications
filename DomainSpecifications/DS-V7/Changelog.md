...
In **DS-V7** a minimal `@context` is provided: If a tool or use-case needs further changes to the `@context` they should be done in a pre-process step. The standard `@context` for **DS-V7** and further explanation can be found in [Context.md](./Grammar/DomainSpecification/Context.md).
